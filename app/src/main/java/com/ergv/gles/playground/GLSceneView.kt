...
class GLSceneView : GLSurfaceView {
    constructor(context: Context) : super(context) {
        init(false, 0, 0)
        setRenderer(Renderer(""))
    }

    constructor(context: Context, scene_tag: String) : super(context) {
        init(false, 0, 0)
        setRenderer(Renderer(scene_tag))
    }
    ...
    private class Renderer : GLSurfaceView.Renderer {
        ...
        constructor(scene_tag: String){
            when (scene_tag) {
                "fractals" -> {
                    mJNIRenderer = FractalPolyhedronsSceneLib()
                }
                "quantized_triangles" -> {
        ...
        override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
            mJNIRenderer.init(width, height)
        }
